...
import com.yktx.check.square.fragment.ReadCardFragment;
...
public class CardView extends FrameLayout {
	private static final int ITEM_SPACE = 14;
	...
	public void setAdapter(CardAdapter adapter, int position) {
	...
	private void ensureFull() {
		while (position < mListAdapter.getCount() && getChildCount() < mMaxVisible) {
			int index = position % mMaxVisible;

			View convertView = viewHolder.get(index);
			final View view = mListAdapter.getView(position, convertView, this);
			view.setOnClickListener(null);
			viewHolder.put(index, view);

			// 添加剩余的View时，始终处在最后
			index = Math.min(mNextAdapterPosition, mMaxVisible - 1);
			Tools.getLog(Tools.d,"kkk", "setScaleX ========== " + (mMaxVisible - index));
			ViewHelper.setScaleX(view, ((mMaxVisible - index) / (float) mMaxVisible) * 0.2f + 0.8f);
			int topMargin = (mMaxVisible - index - 1) * itemSpace;
			Tools.getLog(Tools.d,"aaa", "topMargin ======= " + topMargin);
			...
			Tools.getLog(Tools.d,"aaa", "mNextAdapterPosition ========== " + mNextAdapterPosition);
		}
		// requestLayout();
	}
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		if (topView != null) {
			...
			ReadCardFragment.qiqiuX = topView.getWidth();
			ReadCardFragment.qiqiuY = topView.getHeight();
			Tools.getLog(Tools.i,"kkk", "qiqiuX ========== "+ReadCardFragment.qiqiuX);
			Tools.getLog(Tools.i,"kkk", "qiqiuY ========== "+ReadCardFragment.qiqiuY);
		}
	}
	...
	public int goDown() {
		...
		ViewPropertyAnimator anim = ViewPropertyAnimator.animate(topView).translationY(ViewHelper.getTranslationY(topView) + topView.getHeight())
				.alpha(0).scaleX(1).setListener(null).setDuration(200);
				...
				for (int i = 0; i < count; i++) {
					...
					if (i == count - 1) {
						...
						if ((count == mMaxVisible && i != 0) || count < mMaxVisible) {
							ViewPropertyAnimator.animate(view).translationY(tranlateY).setInterpolator(new AccelerateInterpolator())
		...
		return mListAdapter.getPosition();
	}
	...
	private void bringToTop(final View view) {
		...
		float scaleX = ViewHelper.getScaleX(view) + ((float) 1 / mMaxVisible) * 0.2f;
		float tranlateY = ViewHelper.getTranslationY(view) + itemSpace;
		ViewPropertyAnimator.animate(view).translationY(tranlateY).scaleX(scaleX).setDuration(200).alpha(1)
