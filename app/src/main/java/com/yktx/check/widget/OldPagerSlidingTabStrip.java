...
public class OldPagerSlidingTabStrip extends HorizontalScrollView {
	...
	public OldPagerSlidingTabStrip(Context context, AttributeSet attrs) {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (pager.getAdapter() instanceof IconTabProvider) {
				...
				if (selectedPosition == i) {
					resID = ((IconTabProvider) pager.getAdapter())
							.getSelectPageIconResId(i);
				} else {
					resID = ((IconTabProvider) pager.getAdapter())
							.getPageIconResId(i);
				}
				addIconAndTextTab(i,
						((IconTabProvider) pager.getAdapter())
								.getPageIconResId(i), pager.getAdapter()
	...
	private void addIconAndTextTab(final int position, int resId, String title) {
		...
		tabText.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT,
				LayoutParams.MATCH_PARENT));
	...
	private void updateTabStyles() {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (v instanceof TextView) {
				...
				image.setImageResource(((IconTabProvider) pager.getAdapter())
						.getPageIconResId(i));
				// setAllCaps() is only available from API 14, so the upper case
				// is made manually if we are on a
				// pre-ICS-build
