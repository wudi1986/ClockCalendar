...
public class ChatAddUserToChatGroupService extends Service{

	public ChatAddUserToChatGroupService(String requestType,
			Hashtable<String, String> params, String urlParams,
			ServiceListener serviceListener) {
		super(requestType, params, urlParams, serviceListener);
		// TODO Auto-generated constructor stub
		this.url = UrlParams.IP + Contanst.HTTP_CHAT_ADDUSERTOCHATGROUP;
	...
	void httpSuccess(String reponse) {
		...
		try {
			...
			if (Contanst.HTTP_SUCCESS.equals(retcode)) {//成功获取数据
				...
				serviceListener.getJOSNdataSuccess(str, retcode , Contanst.CHAT_ADDUSERTOCHATGROUP);
			} else {
				String errmsg = (String) result.get("message");
				serviceListener.getJOSNdataFail(erroCodeParse(retcode), errmsg, Contanst.CHAT_ADDUSERTOCHATGROUP);
			}
		} catch (JSONException e) {
			serviceListener.getJOSNdataFail("", "服务器异常", Contanst.CHAT_ADDUSERTOCHATGROUP);
			e.printStackTrace();
		}
	}
	...
	void httpFail(String erro) {
		// TODO Auto-generated method stub
		serviceListener.getJOSNdataFail("", "网络异常", Contanst.CHAT_ADDUSERTOCHATGROUP);
