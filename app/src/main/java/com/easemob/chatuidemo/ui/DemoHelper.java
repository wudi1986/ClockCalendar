...
import com.easemob.chatuidemo.ui.InviteMessage.InviteMesageStatus;
					...
					public Intent getLaunchIntent(EMMessage message) {
						...
						if (chatType == ChatType.Chat) { // 单聊信息
							...
							intent.putExtra("chatType",
									EasemobConstant.CHATTYPE_SINGLE);
						} else { // 群聊信息
							// message.getTo()为群聊id
							...
							if (chatType == ChatType.GroupChat) {
								...
								intent.putExtra("chatType",
										EasemobConstant.CHATTYPE_CHATROOM);
							}
...
//					EasemobConstant.ACTION_GROUP_CHANAGED));
		}
		...
		public void onApplicationReceived(String groupId, String groupName,
			...
			Tools.getLog(Tools.d,TAG, applyer + " 申请加入群聊：" + groupName);
	...
	public class MyContactListener implements EMContactListener {
		...
		public void onContactAdded(List<String> usernameList) {
			...
			// 发送好友变动广播
			broadcastManager.sendBroadcast(new Intent(
					EasemobConstant.ACTION_CONTACT_CHANAGED));
		}

		@Override
		public void onContactDeleted(final List<String> usernameList) {
			// 被删除
			Map<String, EaseUser> localUsers = DemoHelper.getInstance()
					.getContactList();
			for (String username : usernameList) {
		...
		public void onContactInvited(String username, String reason) {
			...
			msg.setTime(System.currentTimeMillis());
			msg.setReason(reason);
			Tools.getLog(Tools.d,TAG, username + "请求加你为好友,reason: " + reason);
			// 设置相应status
		...
		public void onContactAgreed(String username) {
			...
			msg.setFrom(username);
			msg.setTime(System.currentTimeMillis());
			Tools.getLog(Tools.d,TAG, username + "同意了你的好友请求");
		...
		public void onContactRefused(String username) {
			...
			Tools.getLog(Tools.d,username, username + "拒绝了你的好友请求");
		}
	...
	protected void onConnectionConflict() {
		...
		intent.putExtra(EasemobConstant.ACCOUNT_CONFLICT, true);
	...
	protected void onCurrentAccountRemoved() {
		Intent intent = new Intent(appContext, MainActivity.class);
		intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		intent.putExtra(EasemobConstant.ACCOUNT_REMOVED, true);
		appContext.startActivity(intent);
	}
		...
		eventListener = new EMEventListener() {
			...
			public void onEvent(EMNotifierEvent event) {
				...
				if (event.getData() instanceof EMMessage) {
					...
					Tools.getLog(Tools.d, "aaa","receive the event : " + event.getEvent()
							+ ",id : " + message.getMsgId());
				}
				...
				case EventOfflineMessage:
					if (!easeUI.hasForegroundActivies()) {
						Tools.getLog(Tools.d, "aaa","received offline messages");
				...
				case EventNewCMDMessage: {
					...
					Tools.getLog(Tools.d,TAG, "收到透传消息");
					// 获取消息body
					...
					Tools.getLog(Tools.d,TAG, String.format("透传消息：action:%s,message:%s",
	...
	public void saveContact(EaseUser user) {
		EaseUser curUser = contactList.get(user.getUsername());
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					Tools.getLog(Tools.d,TAG, "set contact syn status to true");
