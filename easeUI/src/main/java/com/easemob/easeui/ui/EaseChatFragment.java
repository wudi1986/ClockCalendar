...
public class EaseChatFragment extends EaseBaseFragment implements EMEventListener {
    ...
    protected void setUpView() {
        ...
        String st = getResources().getString(R.string.people);//获得 人）
        if (chatType == EaseConstant.CHATTYPE_SINGLE) { // 单聊
            // 设置标题
            if(EaseUserUtils.getUserInfo(toChatUsername) != null){
                titleBar.setTitle(EaseUserUtils.getUserInfo(toChatUsername).getNick());//他人的名字
            }
            titleBar.setRightImageResource(R.drawable.ease_mm_title_remove);
        } else {
        	titleBar.setBackgroundColor(getResources().getColor(R.color.white));
        	titleBar.isTitleVisity(false);
        	titleBar.isChatTitleLayoutVisity(true);

        	
        	
        	titleBar.setRightImageResource(R.drawable.liaotian_ren);
            if (chatType == EaseConstant.CHATTYPE_GROUP) {
                // 群聊
                EMGroup group = EMGroupManager.getInstance().getGroup(toChatUsername);
                if (group != null){
                	titleBar.setChatTitleName(group.getGroupName());
                	titleBar.setChatTitlePeopleNum(group.getAffiliationsCount()+"人");
//                    titleBar.setTitle(group.getGroupName()+"("+ group.getAffiliationsCount() + st);
                }
                // 监听当前会话的群聊解散被T事件
